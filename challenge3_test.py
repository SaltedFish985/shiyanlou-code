...
class The_Args():
	...
	def operate_args(self.option):
		...
		if option == '-c':
			self.configfile_path = self.args[the_index + 1]
			return self.configfile_path
		elif optin == '-d':
			self.userdatafie_path = self.args[the_index + 1]
			return self.userdatafile_path
		elif option == '-o':
			self.outputfile_path = self.args[the_index + 1]
